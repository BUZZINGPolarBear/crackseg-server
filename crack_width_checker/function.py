...
# 디렉토리 수정
PATH = pm.PATH   # '/Users/jiwonkim/Documents/2022/AICapstoneDesign/aiclops'
        ...
        for col in range(1, img.shape[1] - 1):
            if img[row, col] == 255:
                ...
                for i in range(-8, 0): #'NW'부터 다음 픽셀과 색 비교
                    if int(direction_dict[neighbor_key[i]]) - int(direction_dict[neighbor_key[i + 1]]) == 255:
                        flag += 1
                
                # 분기점(특징점)일때 점과 주변 흰색 점의 방향들을 리스트 start_interval_point_direction_key_list에 추가
                if flag == 1 or flag >= 3:
                    for key in neighbor_key:
                        if direction_dict[key] == 255:
...
def finding_white_until_black(img, img_th, start, direction, LorR=0):
    ...
    d_next, d_next_cl, d_next_fr = direction_func(direction, LorR)
    ...
    while True :
        ...
        # if img_th[next_c[0], next_c[1]] == 255: break
        # if img_th[next_f[0], next_f[1]] == 255: break
...
def profiling_crack_width_func_new(img, img_th, total_chain_list):
    ...
    for segment_block in total_chain_list:
        ...
        for i in range(1, len(segment_block)):
            ...
            temporary_list_segment_block.append(left_width+right_width)
        #
        list_width.append(temporary_list_segment_block)
    #
    return list_width, img
